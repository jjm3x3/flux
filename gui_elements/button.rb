class Button
    ...
    def initialize(window, image, x, y, z, options={}, id=0)
        ...
        @tool_tip_image = options[:tool_tip_image]
    ...
    def draw
        ...
        if intersects && @tool_tip_image
            @tool_tip_image.draw(@window.mouse_x, (@window.mouse_y - @tool_tip_image.height), ZOrder::DIALOG_ITEMS)
        end
    end
