    ...
    def button_up(id)
        if @left_click_down
            ...
            if @are_you_sure_dialog && @are_you_sure_dialog.is_visible?
                if @are_you_sure_dialog.handle_result
                    current_result = @are_you_sure_dialog.get_result
                    @logger.debug "GameGui:button_up: are you sure dialog result is: #{current_result}"
